...
class Planner:
        ...
        while (True):
            ...
            if (path == None):
                ...
                for i in range(len(path)-1):
                    ...
                    self.length_sofar += self.distance(path[i], path[i+1])
                    #print('complete ' + str(i+1), " segment.")
                length_list.append(self.length_sofar)
                with open('length_log.txt', 'w') as f:
    ...
    def update(self):
        ...
        while (Got == False):
            try:
                self.current_map, self.current_free, self.frontiers, count_explore = modify(self.map, self.rr) ## radius = 2
                ...
                explore_list.append(count_explore)
                with open('explore_log.txt', 'w') as f:
...
def main():
    ...
    rospy.Subscriber('/odom', Odometry, Planner1.callback_position)
    #time.sleep(0.1)
